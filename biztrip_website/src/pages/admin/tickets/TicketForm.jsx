...
import {validateEmpty, validateFile, validateForm} from "../../../utils/helper.jsx";
...
const TicketForm = () => {
    ...
    useDocumentTitle(id ? "Sửa vé" : "Thêm mới vé", true)
    ...
    const [formState, setFormState] = useState(initialTicketFormState)
    const [errTitle, setErrTitle] = useState("")
    const [errFare, setErrFare] = useState("")
    useEffect(() => {
        if (id) {
            const test = async () => {
                const res = await dispatch(fetchGetTicketById({id})).unwrap()
            }
            test()
        }
    }, [id])
    ...
    const handleFormReset = () => {
        ...
        setFormState(initialTicketFormState);
        setErrFare("")
        setErrTitle("")
    };
    const handleSubmitForm = async e => {
        ...
        if (!(Object.keys(errors).length === 0)) {
            ...
            await dispatch(fetchSaveTicket({data, navigate, toast}))
        }
    }
    return (
        <>
