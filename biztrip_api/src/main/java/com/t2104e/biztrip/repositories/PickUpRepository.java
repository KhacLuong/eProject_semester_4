...
public interface PickUpRepository extends JpaRepository<PickUpPointEntity, Long> {
    ...
    @Query(value = "SELECT new com.t2104e.biztrip.dto.SchedulePickUpDto(sp.id, l.name, sp.time, sp.status, sp.createdAt, sp.updatedAt)" +
    ...
    List<SchedulePickUpDto> findByKeyword(long scheduleId, String keyword, Sort sort);

    @Query(value = "SELECT new com.t2104e.biztrip.dto.SchedulePickUpDetailDto(sp.id, s, l, sp.time, sp.status, s.createdAt, s.updatedAt) FROM schedule_pick_up_point sp, schedules s , locations l" +
            " WHERE sp.id = ?1 AND sp.scheduleId = s.id AND sp.locationId = l.id")
    Optional<SchedulePickUpDetailDto> findDetailById(long id);
